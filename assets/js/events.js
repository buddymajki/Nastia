...
(function () {
  ...
  if (grid) {
    var html = EVENTS.map(function (ev, i) {
      return (
        ...
        '<dl class="event-meta"><div><dt>Season 1</dt><dd>' + esc(ev.time) + " · Basel · " + esc(ev.language) + "</dd></div></dl>" +
        '<span class="event-card__cta" aria-hidden="true">Details + interest <span class="arr">→</span></span>' +
        "</a>"
      );
    }).join("");
    grid.innerHTML = html;
  ...
  if (detail) {
    ...
    detail.innerHTML =
      ...
      '<h3 style="margin-top:1.1rem">Keep me posted</h3>' +
      "<p style=\"color:var(--ink-soft)\">Online registration opens together with the season dates. Tell us you’re interested in this session and you’ll hear first. Registration is required once dates are live; spots are limited.</p>" +
      '<a class="btn btn--primary" style="width:100%;justify-content:center" href="' + mailto + '">Keep me posted <span class="arr">→</span></a>' +
      '<p style="font-size:0.85rem;color:var(--ink-soft);margin-top:1rem">Write in English, Deutsch, українська or русский. Snacks, coffee and tea are on the house; sessions run about 45 minutes plus open Q&amp;A.</p>' +
      "</aside>" +
      "</div>" +
      "</div></div>";
  }
