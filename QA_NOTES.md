...
## Round 2 (creative redesign) — re-verified

- Re-ran console/network sweep on all 10 URLs: **clean**. Link/asset check: **all resolve**. Weights: home/about ≈1.49MB (extra photo pairs + logos, lazy-loaded), others unchanged.
- True-390px captures (CDP emulation) of home/about/events after redesign: no overflow; bare people rows, stacked pillars, photo pairs and logo strip all render correctly.
- Fixed during round 2: BELONG hero sticker overlapped the photo caption (repositioned); team portraits re-cropped by pixel detection (previous crops were off by up to ~75px for two people).
- Known: `metaRows()` in events.js is now unused (kept for the future detail/list variant) — harmless.
